...
class BaseConnection extends EventEmitter {
  ...
  /**
   * Sets session variables for the connection after the handshake.
   * Called during Connection constructor initialization.
   *
   * If `config.sessionVariables` is provided and is an object, this method
   * constructs and executes a `SET SESSION ...` query to set each variable.
   * For pooled connections (`this._pool` is truthy), it also sets `SESSION_LOCK`.
   *
   * Errors that can occur:
   * - Invalid session variable names (non-alphanumeric/underscore): emits an 'error' event.
   * - Query errors (e.g., database rejects the SET statement): emits an 'error' event.
   * - If the connection is closing or has a fatal/protocol error, session variables are not set.
   *
   * @private
   */
  _setSessionVariables() {
    if (
      ...
      !this.config.sessionVariables ||
      typeof this.config.sessionVariables !== 'object' ||
      Object.keys(this.config.sessionVariables).length === 0
    ) {
      return;
    }

    // Don't set session variables if connection is closing or has fatal error
    if (this._closing || this._fatalError || this._protocolError) {
      return;
    }

    let sessionVarsQuery = 'SET ';
    for (const [sessionVar, value] of Object.entries(
      this.config.sessionVariables
    )) {
      // only allow alphanumeric and underscore in session variable names
      if (!/^[A-Za-z0-9_]+$/.test(sessionVar)) {
        this.emit(
          'error',
          new Error(`Invalid session variable name: ${sessionVar}`)
        );
        return;
      }
      sessionVarsQuery += `SESSION ${this.escapeId(sessionVar)} = ${this.escape(value)},`;
    }
    // Remove trailing comma from the query string
    const finalQuery = sessionVarsQuery.slice(0, -1);
    this.query(finalQuery, (err) => {
      if (err) {
        err.fatal = true;
        this.emit('error', err);
        return;
    ...
    if (this._pool) {
      // For pool connections, set SESSION_LOCK
      this.query('SET SESSION_LOCK = ON', (err) => {
        if (err) {
          err.fatal = true;
          this.emit('error', err);
          return;
