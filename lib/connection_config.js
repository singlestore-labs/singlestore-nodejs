...
class ConnectionConfig {
  constructor(options) {
    ...
    // User-supplied sessionVariables take precedence over defaults.
    // If a key exists in both, the value from options.sessionVariables will override the default.
    if (options.sessionVariables === null) {
      // Allow disabling session variables entirely
      this.sessionVariables = null;
    } else if (
      options.sessionVariables !== undefined &&
      typeof options.sessionVariables !== 'object'
    ) {
      throw new TypeError(
        `sessionVariables must be an object, instead it's a ${typeof options.sessionVariables}`
      );
    } else {
